...
\begin{enumerate}
    ...
    \item Mengukur tingkat presisi model matematika yang dibangun dalam menentukan harga wajar properti sebagai panduan bagi penjual maupun pembeli.
\end{enumerate}
...
\begin{enumerate}
    \item Pembersihan Data: Langkah ini dilakukan untuk menghapus duplikasi dan menangani data kosong. Selain itu, penelitian ini memutuskan untuk mempertahankan data pencilan (outliers) berupa rumah dengan harga ekstrem (di atas Rp 50 Miliar). Tujuannya adalah agar model dapat mempelajari rentang harga pasar Jakarta Selatan secara utuh dan realistis, meskipun keputusan ini berpotensi meningkatkan rata-rata error pada hasil akhir.
...
Sebelum masuk ke perhitungan matematika yang rumit, langkah pertama adalah melihat hubungan antara fitur rumah (seperti luas tanah) dengan harganya. Hubungan ini digambarkan melalui Heatmap pada Gambar \ref{fig:heatmap}.
...
Pada Gambar \ref{fig:heatmap}, warna kotak pertemuan antara \textbf{Luas Tanah (LT)} dan Harga sangat terang. Ini artinya, luas tanah adalah faktor penentu paling utama ($r \approx 0.74$). Semakin luas tanahnya, memiliki kecenderungan kuat harganya semakin mahal. Pengaruh ini lebih besar dibandingkan jumlah kamar tidur atau kamar mandi. Fakta ini menegaskan bahwa di Jakarta Selatan, "tanah" lebih berharga daripada "bangunan". Karena hubungannya berbentuk garis lurus (linear) yang kuat, maka metode \textit{Normal Equation} sangat cocok digunakan.

\subsection{Seberapa Akurat Model Memprediksi?}
Model matematika yang telah dibuat kemudian diuji kemampuannya Prediksi harga pada 101 data rumah baru. Hasil Hasil evaluasi model dapat dilihat pada Tabel \ref{tab:evaluasi}.
...
Nilai $R^2$ sebesar 0.837 bisa diartikan bahwa model ini menjelaskan sekitar 83.7\% pola harga di pasar. Sisanya (sekitar 16\%) dipengaruhi oleh faktor lain yang tidak ada di data, seperti lokasi strategis atau desain rumah. Untuk melihat ketepatannya secara visual, perhatikan Gambar \ref{fig:pred_vs_act}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{perbandingan_harga_aktual_vs_prediksi.png}
    \caption{Grafik Prediksi vs Harga Asli. Titik biru yang menempel pada garis merah berarti Prediksinya    tepat.}
    \label{fig:pred_vs_act}
\end{figure}

Pada Gambar \ref{fig:pred_vs_act}, terlihat titik-titik biru berkumpul rapat di garis merah untuk rumah dengan harga di bawah 30 Miliar. Ini berarti Prediksi model sangat Akurat untuk rumah kelas menengah. Namun, untuk rumah mewah (di atas 50 Miliar), titik-titiknya mulai menyebar menjauhi garis. Artinya, Prediksi model mulai meleset pada rumah-rumah yang sangat mahal.

\subsection{Analisis Kesalahan (Error)}
Kita perlu membedah "kenapa" model bisa mengalami kesalahan prediksi. Analisis ini disebut analisis residual atau analisis sisaan error.

\subsubsection{Pola Kesalahan yang Melebar}
Gambar \ref{fig:residual} memperlihatkan selisih antara harga asli dan harga Prediksi model.
...
Dari grafik penyebaran error , terlihat pola yang menarik: model matematika ini ternyata sangat sangat efektif Prediksi harga rumah kelas menengah (di bawah Rp 30 Miliar) dengan akurat.
Namun, ketika diminta Prediksi harga rumah super-mewah, Prediksi mulai meleset cukup jauh. Hal ini wajar, karena harga rumah mewah seringkali ditentukan oleh faktor 'gengsi', nilai seni, atau keunikan desain yang tidak bisa dihitung hanya dengan rumus luas tanah dan bangunan semata.

\subsubsection{Keseimbangan Kesalahan}
Terakhir, kita melihat apakah model cenderung Prediksi "ketinggian" atau "kerendahan" melalui Gambar \ref{fig:hist_error}.
...
Meskipun kesalahannya membesar pada rumah mewah, Gambar \ref{fig:hist_error} menunjukkan kurva berbentuk lonceng sempurna yang puncaknya ada di angka 0. Ini kabar baik. Artinya, kesalahan model bersifat acak dan seimbang (adil). Model tidak memiliki Kecenderungan salah suka melebih-lebihkan harga atau merendahkan harga secara sengaja. Kadang Prediksi sedikit di atas, kadang sedikit di bawah, tapi rata-ratanya pas.

\subsection{Interpretasi Model dan Simulasi Prediksi}

Salah satu keunggulan utama algoritma Regresi Linear adalah transparansi modelnya (\textit{white-box model}). Tidak seperti \textit{Deep Learning} yang seringkali sulit diinterpretasikan, keputusan model ini dapat diturunkan menjadi persamaan matematika yang jelas. 

Berdasarkan hasil pelatihan (\textit{training}) menggunakan Python, solusi numerik menggunakan metode \textit{Normal Equation} menghasilkan vektor parameter optimal $\hat{\boldsymbol{\theta}}$. Model prediksi harga rumah ($\hat{y}$) dapat dituliskan kembali dalam bentuk persamaan linear sebagai berikut:

\begin{equation}
    \label{eq:final_model_theta}
    \begin{split}
    \hat{y} &= \theta_0 + \theta_1 \cdot x_{\text{Luas tanah}} + \theta_2 \cdot x_{\text{Luas Bangunan}} \\
    &\quad + \theta_3 \cdot x_{\text{Kamar Tidur}} + \theta_4 \cdot x_{\text{Kamar Mandi}} \\
    &\quad + \theta_5 \cdot x_{\text{Garasi}}
    \end{split}
\end{equation}

\noindent Dimana nilai parameter $\boldsymbol{\theta}$ yang diperoleh dari hasil komputasi adalah:
\begin{itemize}
    \item \textbf{Intersep ($\theta_0$):} -3533645023.45 atau -3.53 Miliar.
    \item \textbf{Koefisien Luas Tanah ($\theta_1$):} 21522032.48 atau 21.52 Juta. 
    \item \textbf{Koefisien Luas Bangunan ($\theta_2$):} 12081321.81 atau 12.08 Juta. 
    \item \textbf{Koefisien Kamar Tidur ($\theta_3$):} 268049808.06 atau 268.05 Juta.
    \item \textbf{Koefisien Kamar Mandi ($\theta_4$):} 298087646.55 atau 298.09 Juta. 
    \item \textbf{Koefisien Garasi ($\theta_5$):} 1878969867.69 atau 1.88 Miliar.
\end{itemize}

\subsubsection{Simulasi Perhitungan Manual}
Untuk memvalidasi akurasi persamaan di atas, dilakukan perhitungan manual pada satu sampel data uji (Indeks ke-5 pada data test) dengan spesifikasi: 
Luas Tanah 1186 $m^2$, 900 Luas Bangunan $m^2$, 6 Kamar Tidur, dan 5 Kamar Mandi, Memiliki Garasi dengan harga Rp 34 M.

\begin{align*}
    \hat{y} &= -3533645023.45 + (21522032.48 \cdot 1186) \\
            &\quad + (12081321.81 \cdot 900) + (268049808.06 \cdot 6) \\
            &\quad + (298087646.55 \cdot 5) + (1878969867.69 \cdot 1) \\[10pt]
    \hat{y} &= 37842382073.88 \\
            &\approx \text{Rp } 37.84 \text{ Miliar}
\end{align*}

Bandingkan hasil hitungan manual ini dengan harga aktual pasar sebesar \textbf{34.00 Miliar}. Terdapat selisih prediksi sebesar \textbf{3.84 Miliar} (sekitar 11\%). Hal ini mengonfirmasi analisis residual sebelumnya bahwa model cenderung memiliki varians error yang lebih besar pada segmen rumah mewah (harga tinggi) dibandingkan rumah kelas menengah, namun estimasi ini masih memberikan gambaran taksiran nilai aset yang cukup representatif.

\subsubsection{Validasi pada Data Sampel Lain}
Penerapan persamaan tersebut secara massal pada data uji lainnya dapat dilihat pada Tabel \ref{tab:sampel}. Tabel ini memperlihatkan perbandingan antara harga asli (\textit{Ground Truth}) dengan harga prediksi model.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{cccc}
        \toprule
        Luas ($m^2$) & Harga Asli (M) & Prediksi (M) & Selisih (M) \\
        \midrule
        216 & 18.00 & 9.90 & +8.10 \\
        123 & 2.99 & 3.01 & -0.02 \\
        246 & 8.60 & 7.69 & +0.91 \\
...
baris kedua (Luas 123 $m^2$): harga asli 2.99 M, Prediksi model 3.01 M. Selisihnya sangat tipis, nyaris sempurna.
Namun, baris pertama (Luas 216 $m^2$): harga asli sangat tinggi (18 M), tapi model hanya Prediksi 9.9 M. Selisihnya mencapai 8 Miliar. Ini membuktikan bahwa untuk rumah kecil yang harganya tidak wajar (mungkin karena lokasi sangat premium), model "kalah" karena hanya mengandalkan luas tanah sebagai patokan utama.
...
\begin{enumerate}
    \item Tanah Adalah Kunci Objektivitas: Penelitian ini menegaskan bahwa harga rumah tidak terbentuk secara acak. Penulis menemukan bahwa \textit{Luas Tanah} memegang peranan paling dominan ($r \approx 0.74$) dalam menentukan harga. Artinya, penggunaan rumus matematika berbasis data fisik dapat menjadi solusi ampuh untuk menghilangkan Subjektivitas penentuan harga atau subjektivitas yang sering membingungkan penjual dan pembeli.
    
    \item Akurasi dan Batas Kemampuan: Model yang dibangun berhasil memahami pola harga pasar dengan cukup baik (akurasi 83.7\%). Model ini Memiliki tingkat akurasi tinggi untuk memprediksi harga rumah kelas menengah. Namun, penulis menemukan batasan wajar: model mulai Mengalami penurunan performa saat memprediksi rumah super-mewah. Hal ini terjadi karena harga rumah mewah seringkali dipengaruhi oleh nilai seni atau gengsi yang tidak bisa diukur hanya dengan Variabel luas tanah.
\end{enumerate}
...
\end{verbatim}
    ...
    \item \textbf{Prediksi Manual}
\begin{lstlisting}[language=Python]
def persamaan_linear_prediksi_rumah(X, theta):
    return (theta[0] + 
            theta[1] * X[0] + 
            theta[2] * X[1] + 
            theta[3] * X[2] + 
            theta[4] * X[3] + 
            theta[5] * X[4])
X_contoh, y_contoh = X_test[5], y_test[5]
y_contoh_pred = persamaan_linear_prediksi_rumah(X_contoh, theta)
features = ['Luas Tanah', 'Luas Bangunan', 'Kamar Tidur', 'Kamar Mandi', 'Garasi']
for i in range(len(X_contoh)):
    print(f'{features[i]} = {X_contoh[i]}', end=', ')
    if i == len(X_contoh) - 1:
        print(f"\nHarga Rumah Aktual = Rp {(y_contoh/1_000_000_000)} M", )
        print(f"Harga Rumah Prediksi = Rp {(y_contoh_pred)} atau Rp {(y_contoh_pred/1_000_000_000)} M")
\end{lstlisting}
    \noindent\textbf{Output:}
\begin{verbatim}
Luas Tanah = 1186, Luas Bangunan = 900, Kamar Tidur = 6, Kamar Mandi = 5, Garasi = 1, 
Harga Rumah Aktual = Rp 34.0 M
Harga Rumah Prediksi = Rp 37842382073.88126 atau Rp 37.84238207388126 m
\end{verbatim}
